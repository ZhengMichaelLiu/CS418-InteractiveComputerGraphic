...
class TriMesh {
    ...
    computeAABB() {
        for (i = 0; i < this.numVertices; i++) {
            if (this.vBuffer[i * 3] > this.maxXYZ[0]) {
                this.maxXYZ[0] = this.vBuffer[i * 3];
            } else if (this.vBuffer[i * 3] <= this.minXYZ[0]) {
                this.minXYZ[0] = this.vBuffer[i * 3];
            }

            if (this.vBuffer[i * 3 + 1] > this.maxXYZ[1]) {
                this.maxXYZ[1] = this.vBuffer[i * 3 + 1];
            } else if (this.vBuffer[i * 3 + 1] <= this.minXYZ[1]) {
                this.minXYZ[1] = this.vBuffer[i * 3 + 1];
            }

            if (this.vBuffer[i * 3 + 2] > this.maxXYZ[2]) {
                this.maxXYZ[2] = this.vBuffer[i * 3 + 2];
            } else if (this.vBuffer[i * 3 + 1] <= this.minXYZ[2]) {
                this.minXYZ[2] = this.vBuffer[i * 3 + 2];
            }
    ...
    loadFromOBJ(fileText) {
        ...
        var length_of_data = lines.length;
        for (var i = 0; i < length_of_data; i++) {
            ...
            if (current_line[0] == '#') {
                ...
                this.fBuffer.push(parseInt(current_line[2]) - 1);
                this.fBuffer.push(parseInt(current_line[3]) - 1);
                this.fBuffer.push(parseInt(current_line[4]) - 1);
            } else if (current_line[0] == 'v') {
                this.numVertices = this.numVertices + 1;
                this.vBuffer.push(parseFloat(current_line[1] * 0.2));
                this.vBuffer.push(parseFloat(current_line[2] * 0.2));
                this.vBuffer.push(parseFloat(current_line[3] * 0.2));
            }
        }
    ...
    drawTriangles() {
        ...
        gl.vertexAttribPointer(shaderProgram.vertexNormalAttribute,
            ...
            gl.FLOAT, false, 0, 0);
        gl.uniform1i(shaderProgram.ucheckSky, false);
        //Draw 
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.IndexTriBuffer);
